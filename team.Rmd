...
biography <- biography %>% mutate_at(.vars = c(1,2,4,5,6), .funs = list(as.factor)) %>% mutate_at(.vars = c(3), .funs = funs(as.numeric))
bioClean <- biography
str(bioClean)
```
...
dat2 <- dat1 %>%
        ...
        pivot_wider(id_cols = c(1:2), names_from = Opponent_Opponent, values_from = value)
